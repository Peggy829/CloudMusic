...
import {getPlayList,getAlbum,getArtistsSongUrl,getTopList} from '../api/index'
...
export default {
  ...
  created(){
    ...
    if(this.$route.params.type === 'personalized'){
      ...
      getArtistsSongUrl({id: this.$route.params.id })
        .then((data) => {
          // console.log(data);
          // console.log(this.$route.params.id);
  ...
  mounted(){
    ...
    this.$refs.scrollview.scrolling((offsetY) => {
      ...
      if(offsetY < 0){
        ...
        /*
        注意：高斯模糊非常消耗性能，不推荐在移动端使用，如果一定要使用，建议只设置一次
        */ 
      }else{
        // 向下滚动
